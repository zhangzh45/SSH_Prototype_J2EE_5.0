...
public class RoleDAO extends HibernateDaoSupport {
	...
	public List findByRoleName(String roleName, Object value) {
		log.debug("finding Role instance with property: " + roleName
				+ ", value: " + value);
		try {
			String queryString = "from Role as model inner join model.userRoles ur inner join ur.user u inner join u.userSpecSers us inner join us.service " +
					"where  model."+roleName+" = ?";
	...
	public List findUserByRoleName(String roleName, Object value) {
		log.debug("finding Role instance with property: " + roleName
				+ ", value: " + value);
		try {
			String queryString = "from Role as model inner join model.userRoles ur inner join ur.user " +
					"where  model."+roleName+" = ?";
			return getHibernateTemplate().find(queryString, value);
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
