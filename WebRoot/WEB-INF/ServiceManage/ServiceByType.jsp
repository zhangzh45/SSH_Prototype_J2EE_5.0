...
<body >
			...
			<div >
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<form name="form2" action="" method="post">
						  <div class="navbar">
						  	<div class="navbar-inner">
						  		...
						  		<ul class="nav">
						  			<li  class="active"><a onclick="form2.action='serviceByType.action';form2.submit()"><s:text name="ServiceClassification.TechnicalClassification"></s:text></a></li>
						  			<li><a onclick="form2.action='serviceByBusiness.action';form2.submit()"><s:text name="ServiceClassification.BusinessClassification"></s:text></a></li>
						...
						<form id="form3" method="post" action="ServiceOfSelectedType.action">
							<div class="control-group">
								<div class="controls"><s:text name="ServiceClassification.TechnicalHint"></s:text>
									<select id="selecttypes">  <!-- 审核通过的服务 -->
										<option>ALL</option>
										<s:iterator value="servicetypes" status="L" var="servicetypes">
											<option><s:property/></option>
										</s:iterator>
									</select>
									<input id="selectedtype" name="selectedtype" type="hidden" value=<s:property value="selectedtype"/>>
								</div>
							</div>
						</form>
	...
	<script>
		 jQuery(document).ready(function() {       
		  ...
		  // alert(document.getElementById("selectedtype").value);
		   if(document.getElementById("selectedtype").value == "null"){
		  		document.getElementById("selecttypes")[0].checked = true;
		   }else{
		   		var selected = document.getElementById("selectedtype").value;
		  		document.getElementById("selecttypes").value = selected;
		   }
		});
		
		$('#selecttypes').change(function(){ 
			var servicetypeobj = document.getElementById("selecttypes");
			var servicetypeindex = servicetypeobj.selectedIndex;
			var servicetypevalue = servicetypeobj.options[servicetypeindex].value;
			document.getElementById("selectedtype").value = servicetypevalue;
			form3.submit();
			/*$.ajax({
				url		:	"ServiceOfSelectedType.action",
				data	:	"selectedtype="+servicetypevalue,
				type	:	"post",
				dataType:	"json",
				success	:	updateTable
			});*/
		...
		function updateTable(queryServiceresult){
		
			var services = JSON.parse(queryServiceresult);
			var tbody = $('#tbody');
			tbody.empty();
			$(services).each(function() {
				var str = $('<tr/>');
				str.append("<td>"+this.serviceid+"</td>");
				str.append("<td>"+this.servicename+"</td>");
				str.append("<td>"+this.servicetype+"</td>");
				str.append("<td>"+this.relatedbusiness+"</td>");
				str.append("<td>"+this.servicetarget+"</td>");
				str.append("<td>"+this.servicerange+"</td>");
				str.append("<td>"+this.serviceaddress+"</td>");
				str.append("<td>"+this.servicedesc+"</td>");
				tbody.append(str);
			});
		}
